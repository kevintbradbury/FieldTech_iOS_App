...
class FanMenuScene {
        ...
        if let uiImage = UIImage(named: button.image) {
            menuIcon = Image(
                ...
                place: Transform.move(
                    dx: -Double(fanMenu.radius * 1.5),
                    dy: -Double(fanMenu.radius * 1.5)
                ),
    ...
    class func createFanButtonNode(button: FanMenuButton, fanMenu: FanMenu) -> Group {
        ...
        if let uiImage = UIImage(named: button.image) {
            let image = Image(
                src: button.image,
                w: Int(fanMenu.radius * 1.5),
                h: Int(fanMenu.radius * 1.5),
                place: Transform.move(
                    dx: -Double(fanMenu.radius * 0.75),
                    dy: -Double(fanMenu.radius * 0.75)
                ),
